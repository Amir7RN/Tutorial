...
class ActorCriticPage(Page):
    ...
    def __init__(self, parent=None):
        ...
        ds.add(body(
            (
                "<b>Stochastic actor.</b> The network outputs the parameters of a distribution over actions — "
                "for a continuous action, typically a mean and a standard deviation — and the action is "
                "<b>sampled</b> from it. Written π(a|s). Exploration is built in: the policy is random, so it "
                "tries things. Two consequences follow. Stochastic does not mean on-policy: PPO uses "
                "current-policy rollouts, while SAC uses off-policy replay. Gradients may use likelihood ratios "
                "or reparameterisation, depending on the algorithm."
            )))
        ...
        ds.add(body(
            (
                "<b>Deterministic actor.</b> The network outputs <b>the action itself</b>. Written μ(s) = a. "
                "Given the same state it returns the same 15.5 N·m forever. There is no sampling and no "
                "distribution, so exploration is not automatic — it has to be <b>added from outside</b>, usually"
                " as Gaussian noise on the output during training only. In exchange the critic’s local "
                "derivative is <b>computed</b>, but the policy gradient is still estimated from sampled states "
                "and an approximate critic: you differentiate the critic with respect to the action and push."
            )))
        ...
        ds.add(callout(
            (
                "<b>Why DDPG uses a deterministic actor.</b> It outputs a bounded action directly and learns "
                "from the critic’s action derivative. This avoids an explicit runtime action search and supports"
                " the chosen off-policy update. The derivative is of an approximate critic, and averaging over "
                "sampled states still estimates the policy gradient.<br><br>Stochastic methods such as SAC and "
                "PPO also handle continuous control. They can be evaluated with a deterministic mean action when"
                " appropriate. Sample efficiency, exploration and robustness depend on the complete algorithm, "
                "task and implementation, not just whether the actor is deterministic."
            ), "key"))
        self.add(ds)
        ...
        fam.add(body(
            (
                "<b>Keep two distinctions separate.</b> Deterministic versus stochastic describes how the actor "
                "chooses an action. On-policy versus off-policy describes the relationship between the "
                "data-collecting policy and the policy being learned. DDPG is deterministic and off-policy; SAC "
                "is stochastic and off-policy; PPO is usually stochastic and on-policy. A V-critic or Q-critic "
                "is an algorithm choice, not a universal restriction."
            ), dim=True))
        self.add(fam)
    ...
    def _redraw_dist(self):
        ...
        if sn <= 1e-6:
            self.tB.setText(
                (
                    "<b>Noise at zero: this actor has no added action exploration.</b> It can still change through "
                    "training on retained experience. The risk is poor coverage: without trying other actions, the "
                    "critic may not discover that they are better. A stochastic actor has policy randomness, but "
                    "randomness alone does not guarantee useful exploration."
                ))
        else:
            self.tB.setText(
...
class DDPGPage(Page):
    ...
    def __init__(self, parent=None):
        ...
        eq.add(callout(
            (
                "<b>There is no label anywhere in reinforcement learning.</b> In supervised learning a human "
                "wrote \"cat\" next to the picture. Here nobody knows what Q(s,a) should be — that is the whole "
                "problem. So we build a stand-in out of one real number and one guess:<br><br>&nbsp;&nbsp;• "
                "<b>r</b> — the reward that <i>actually happened</i>. This is the only fact in the equation, and"
                " it is why the method works at all: every target is anchored to at least one real "
                "measurement.<br>&nbsp;&nbsp;• <b>γQ′(s′, μ′(s′))</b> — the target networks' current opinion "
                "about everything after that. A guess.<br><br>We then treat y as though it were handed down, and"
                " regress the critic onto it. Each update the guess is one real reward less wrong than it was, "
                "and that slow leaking of real rewards backward through time <b>is</b> the learning. It is the "
                "Bellman equation from page 62, enforced by gradient descent instead of by assignment."
            ), "key"))
        ...
        eq.add(body(
            (
                "Read the two factors. The <b>critic</b> is asked a question it is uniquely able to answer: "
                "<i>at the action we are currently taking, which direction in action-space increases value?</i> "
                "That is ∂Q/∂a, and getting it costs one backward pass through the critic — not to its weights, "
                "but all the way through to its <b>input</b>. The <b>actor</b> then answers: <i>how do I change "
                "my weights to move the output that way?</i> Multiply to obtain a local ascent direction for the"
                " current critic. A finite update need not improve predicted Q, and improved predicted Q need "
                "not improve true return.<br><br>It is gradient <b>ascent</b>, which is why the implementation "
                "negates the term before handing it to a descent optimiser. And notice what never happens: no "
                "action is ever enumerated, compared, or maximised over. The actor is dragged toward the maximum"
                " a small step at a time, forever."
            ), dim=True))
        eq.add(body(
        ...
        rb.add(body(
            (
                "A transition (s, a, r, s′) is a <b>fact about the environment</b>: from there, that action paid"
                " that much and led here. That fact does not expire when the policy changes. So DDPG keeps a "
                "finite replay buffer and trains on samples from retained experience.<br><br>Two separate "
                "benefits, and people usually only name the first:<br>&nbsp;&nbsp;• <b>Decorrelation.</b> "
                "Consecutive steps look almost identical, and a gradient method fed correlated samples is "
                "effectively taking one very large step in one direction. Uniform sampling from a big buffer "
                "breaks that.<br>&nbsp;&nbsp;• <b>Sample reuse.</b> Each real interaction is used hundreds of "
                "times. On a robot where a sample is a stride taken by an actual person, this is not an "
                "optimisation — it is the difference between a feasible experiment and an impossible one. PPO, "
                "being on-policy, reuses its rollout for several optimisation epochs, then collects a fresh "
                "batch; that is why it is the algorithm of simulators."
            )))
        rb.add(callout(
            (
                "<b>Replay belongs to the off-policy update rule.</b> DDPG’s Q target can learn from transitions"
                " collected by older behavior policies. SAC also uses replay with a stochastic actor. A "
                "deterministic actor is not what makes replay valid, and V estimates can also use off-policy "
                "corrections in other algorithms."
            ), "key"))
        self.add(rb)
        ...
        hn.add(callout(
            (
                "<b>Three pages of this tutor exist to unpack that paragraph.</b> Page 84 derives the "
                "multiply-along-a-path, sum-across-paths rule and checks it against a brute-force numerical "
                "derivative. Page 89 takes this exact handoff apart line by line — including what Adam does to "
                "the magnitude, which is not what most people expect. Page 91 opens the critic and asks where "
                "the action should enter it, which turns out to be a question about how many weight-and-slope "
                "factors sit between the action and the value."
            ), "good"))
        self.add(hn)
...
class MDPandFamilyPage(Page):
    ...
    def __init__(self, parent=None):
        ...
        mk.add(callout(
            (
                "<b>What DDPG’s target evaluates.</b> The next action is μ′(s′), so the target uses Q′(s′, "
                "μ′(s′)); it does not perform an exact max during the backup. Actor training seeks actions with "
                "higher predicted Q. A deterministic policy can choose one of several equally good actions; DDPG"
                " does not assume every state has a unique optimum."
            ),
            "key"))
        self.add(mk)
        ...
        cl.add(_table(
            ...
            [("1. Model known exactly",
             ...
             ("2. Model unknown → learn one",
              ...
              (
                  "Model-based RL: fit a dynamics model from data, then plan or generate imagined rollouts against"
                  " it. PILCO, MBPO, Dreamer, MuZero’s learned dynamics (AlphaZero instead uses known game rules)."
              ),
        ...
        cl.add(body(
            (
                "<b>Two refinements on the informal version.</b> First, \"if we know the dynamics RL is useless\" "
                "is too strong — a known model makes <i>planning</i> available, and planning is usually better, "
                "but a stochastic model with a huge branching factor can still be easier to attack by sampling "
                "than by exhaustive search. Say \"unnecessary\" rather than \"useless\". Second, the "
                "model-based/model-free line is about <b>whether an explicit dynamics/reward model is used for "
                "planning or learning</b> (the model may be known or learned), not about whether a network "
                "appears. DDPG has two networks and neither predicts a next state, so it is model-free — it goes"
                " straight for the value."
            ), dim=True))
        cl.add(callout(
    ...
    def _pick(self):
        ...
        msgs = [
            ...
            (
                "<b>SAC.</b> A deterministic actor commits to a single action per state, which is exactly wrong "
                "when the value surface has several peaks of similar height — it will pick one arbitrarily and "
                "lose the others. SAC's entropy bonus explicitly rewards keeping probability mass on all of the "
                "good options, which encourages exploration; a usual single-Gaussian SAC actor does not "
                "guarantee a multi-modal action distribution."
            ),
...
class DeployRLPage(Page):
    ...
    SUBTITLE = ((
                    "A control task needs a verified execution-time bound. Keep training outside that task unless it"
                    " meets the same deadline guarantees."
                ))
    SECTION = SECTION
