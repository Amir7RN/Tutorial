...
class ControlProblemPage(Page):
    ...
    def __init__(self, parent=None):
        ...
        f.add(body(
            (
                "<table cellpadding='8'><tr><td width='26'><b>1</b></td><td><b>What does the thing do on its "
                "own?</b><br>Push it and watch. This is the <b>plant</b>, and its unforced behaviour is the "
                "<b>open-loop response</b>. Characterising it means answering: how fast, how far, does it "
                "overshoot, can it be destabilised. <i>Pages 3–15 are entirely "
                "this.</i></td></tr><tr><td><b>2</b></td><td><b>What do you want instead?</b><br>Written as "
                "<b>specifications</b>: settle within 200 ms, no more than 5% overshoot, zero steady-state error"
                " against gravity, recover from a kick in half a second, and stay that way when the payload "
                "doubles. Notice these are all statements about the <i>transient</i> and the <i>steady state</i>"
                " — the two things the next pages teach you to read.</td></tr><tr><td><b>3</b></td><td><b>How "
                "far apart are those two?</b><br>This is where order matters. Some gaps close with a single gain"
                " knob. Some cannot be closed by any gain and need a different kind of controller. Some cannot "
                "be closed at all without changing the mechanics — and knowing which case you are in, "
                "<i>before</i> you start tuning, is most of the skill.</td></tr><tr><td><b>4</b></td><td><b>Add "
                "a controller and close the loop.</b><br>P, PD, PI, PID, lead, lag, state feedback. <i>Pages "
                "16–20.</i> And the sentence that makes it all one subject: <b>a controller does not change the "
                "plant. It moves the plant's poles.</b> The metal is unchanged; feedback rearranges where the "
                "roots of the closed-loop equation sit.</td></tr></table>"
            )))
        self.add(f)
        ...
        pp.add(body(
            (
                "<b>The standard name for this is observability, and it is a whole page later on (page 20).</b> "
                "A state that produces no signature at your sensor is called <b>unobservable</b>, and a transfer"
                " function silently drops every unobservable mode — which is why the <i>transfer-function "
                "order</i> can be lower than the number of state variables. The full model is called a "
                "<b>realisation</b>; the reduced one that keeps only what is both drivable and visible is the "
                "<b>minimal realisation</b>, and that is what a transfer function always is."
            )))
        pp.add(callout(
        ...
        rm.add(body(
            (
                "<table cellpadding='7'><tr><td width='34'><b>3</b></td><td><b>Measuring a real joint.</b> Where"
                " J, b and k actually come from. Four bench experiments, and the regression that gets all of "
                "them at once — because every number on every later page is one you had to go and "
                "measure.</td></tr><tr><td><b>4</b></td><td><b>How software changes physics.</b> The one that "
                "has to be settled before poles mean anything: you cannot change J with a gain, so how does a "
                "gain change the behaviour? Answer: it adds a term to the equation of "
                "motion.</td></tr><tr><td><b>5</b></td><td><b>What a first-order system is.</b> One storage "
                "element. How you identify order on real hardware, and what the answer buys "
                "you.</td></tr><tr><td><b>6</b></td><td><b>The time constant and the pole.</b> How fast, "
                "expressed three equivalent ways. Where 0.63 comes from. Why the current loop runs at 20 "
                "kHz.</td></tr><tr><td><b>7</b></td><td><b>The pole at the origin.</b> A storage element with no"
                " dissipation. Why position is second order, and why the I term "
                "works.</td></tr><tr><td><b>8</b></td><td><b>First order in frequency.</b> Shake it with a sine "
                "instead of stepping it. Bandwidth, dB, phase lag — the language every datasheet and every "
                "loop-shaping argument uses.</td></tr><tr><td><b>9</b></td><td><b>The s-plane.</b> The two axes,"
                " what the imaginary part physically is, and the second storage element that creates "
                "it.</td></tr><tr><td><b>10</b></td><td><b>Second-order systems.</b> ω<sub>n</sub> and ζ — and "
                "the fact that the K and B of an impedance controller <i>determine</i> ω<sub>n</sub> and ζ "
                "together with inertia. This is where every robot joint you will tune actually "
                "lives.</td></tr></table>"
            )))
        rm.add(callout(
