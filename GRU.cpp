...
public:
    ...
    MatrixXf Wr, Wz, Wn; // weight for Inputs side: forget (cont to % of forgetting of long term), input (cont to % of updating long term), 
                            // output (cont to output//updating short term), and cell gates (cont to gnerating new/update for long term) --> inputs
    MatrixXf Ur, Uz, Un; //wieght for Hidden state side: forget, input, output, and cell gates --> hidden state
    VectorXf br, bz, bn;

    VectorXf h; // Hidden state and cell state
    ...
    void forward(const VectorXf &x_t) {
        // 1. Reset Gate (r_t) and Update Gate (z_t)
        VectorXf r_t = (Wr * x_t + Ur * h + br).unaryExpr([this](float elem) { return sigmoid(elem); });
        VectorXf z_t = (Wz * x_t + Uz * h + bz).unaryExpr([this](float elem) { return sigmoid(elem); });
        // 2. Candidate Hidden State (n_t) 
    // Notice the Reset gate (r_t) multiplies the previous hidden state (h)
        VectorXf n_t = (Wn * x_t + Un * (r_t.array() * h.array()).matrix() + bn).array().tanh();


        h = (1.0f - z_t.array()) * n_t.array() + z_t.array() * h.array();
    }
